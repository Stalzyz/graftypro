#!/usr/bin/env tsx
/**
 * Inserts Pummy's E-Commerce Catalog Flow:
 * Trigger -> E-Commerce Catalog Node (10 products) -> Direct Razorpay Payment Gateway per product -> Contact Form -> Confirmation
 */
...
// 2. E-COMMERCE CATALOG NODE (10 Products from Pummy's Store)
const catalogNodeId = 'catalog_products';
nodes.push({
  id: catalogNodeId,
  type: 'catalog',
  data: {
    label: 'Pummy\'s Store Catalog (10 Products)',
    buttonTitle: 'Buy Now',
...
// 5. 10 PAYMENT GATEWAY NODES — CONNECT EACH CATALOG PRODUCT DIRECTLY TO PAYMENT
...
products.forEach((prod, i) => {
  ...
  // Edge 1: Catalog item -> Payment Gateway Node for that product
...
async function main() {
  ...
  console.log(`✅ Catalog Flow updated successfully: ${flow.id}`);
