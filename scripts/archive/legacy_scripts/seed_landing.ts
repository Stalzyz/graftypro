...
const homeContent = {
  title: 'Grafty Homepage - V2 Engine',
  slug: 'home',
  status: 'PUBLISHED',
  banner_config: {
    ...
    create: [
      {
        ...
        is_active: true,
        content: {
          badgeText: "Official WhatsApp Business API Platform <Sparkles size={12} class='inline text-green-500' />",
...
async function seed() {
  try {
    console.log("Seeding home landing page with V2 Blocks...");

    // delete existing home
    const existing = await prisma.landingPage.findUnique({ where: { slug: 'home' } });
    if (existing) {
      await prisma.landingSection.deleteMany({ where: { landing_page_id: existing.id } });
    ...
    console.log("Created successfully with ID: ", page.id);
  } catch (e) {
