...
async function run() {
    ...
    try {
        ...
        console.log('Fetching details for flow:', targetId, sortedFlows[0].name);
        
        const detailsRes = await axios.get(`${BASE_URL}/${targetId}?fields=validation_errors,status,name`, {
            headers: { Authorization: `Bearer ${token}` }
        });
