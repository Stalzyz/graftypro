...
export function UniversalCrmBoard({
    ...
    const onDragEnd = async (result: any) => {
        ...
        // Find lead and stage to check if anything changed
        const lead = leads.find((l: any) => l.id === draggableId);
        if (lead && lead.stage_id === destination.droppableId) return;

        try {
            const res = await fetch(`/api/crm/leads/${draggableId}`, {
                method: "PATCH",
                            ...
                            <Droppable key={stage.id} droppableId={stage.id}>
                                {(provided) => (
                                    <div
                                        ...
                                        className="w-80 flex-shrink-0 bg-white border border-slate-200 rounded-[2rem] shadow-sm flex flex-col max-h-full transition-colors"
                                    >
                                        <div className="p-5 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white/90 backdrop-blur-md rounded-t-[2rem] z-10 text-slate-900">
                                            <div className="flex items-center gap-3">
                                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: stage.color }} />
                                                <h4 className="font-black tracking-tight">{stage.name}</h4>
                                            </div>
                                            <span className="text-[10px] font-black bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{stageLeads.length}</span>
                                        </div>

                                        <div className="p-4 flex-1 overflow-y-auto space-y-4 custom-scrollbar min-h-[150px]">
                                                ...
                                                <Draggable key={lead.id} draggableId={lead.id} index={index}>
                                                    {(provided, snapshot) => (
                                                        <div
                                                            ...
                                                            className={`bg-white border border-slate-200 p-4 rounded-3xl shadow-sm hover:shadow-md transition-all cursor-pointer group ${snapshot.isDragging ? 'ring-2 ring-blue-500 ring-offset-2' : ''}`}
                                                        >
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    {lead.contact_id ? (
                                                                        <button
                                                                            ...
                                                                            className="text-white bg-emerald-500 flex items-center gap-1 text-[9px] font-black uppercase hover:bg-emerald-600 px-2 py-1 rounded-md transition-colors"
                                                                        >
