...
export function TrialExpiredGate() {
    ...
    const fetchTrial = useCallback(async () => {
        try {
            ...
            setTrial({ status: "paid", days_left: 0, trial_expired: false }); // Fail open to avoid false locks
        }
    }, []);

    useEffect(() => { fetchTrial(); }, [fetchTrial]);

    if (trial.status === "loading" || !trial.trial_expired) return null;
            ...
            <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-10 text-center relative overflow-hidden">
                ...
                <div className="bg-slate-50 rounded-2xl p-5 mb-8 border border-slate-100">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {[
                            { name: "Lite Chat", price: "₹999/mo", tag: "Templates + Chat" },
                            { name: "Prime Starter", price: "₹1,999/mo", tag: "Full Automation" }
                        ].map(p => (
                            <div key={p.name} className="bg-white border border-slate-200 rounded-xl p-3 text-left">
                                <p className="text-xs font-black text-slate-900">{p.name}</p>
                                <p className="text-lg font-black text-[#27954D]">{p.price}</p>
                                <p className="text-[10px] text-slate-400 font-medium">{p.tag}</p>
                            </div>
                        ))}
                    </div>
                </div>
    ...
    return (
        <div className={`w-full flex items-center justify-between px-6 py-2.5 text-sm font-bold ${urgent ? "bg-orange-500 text-white" : "bg-amber-50 text-amber-900 border-b border-amber-200"}`}>
            <div className="flex items-center gap-3">
                ...
                <Link href="/pricing" className={`underline font-black ${urgent ? "text-white" : "text-amber-700"}`}>
                    Upgrade Now →
                </Link>
            </div>
